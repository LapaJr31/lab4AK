...
MODULE_ALIAS("hello2"); // Псевдонім модуля
...
static int __init hello2_init(void) {
  ...
  // Перевірка введеного значення print_count
  if (print_count == 0 || (print_count >= 5 && print_count <= 10)) {
    ...
    return -EINVAL; // Повернення помилки, якщо значення некоректне
  }
